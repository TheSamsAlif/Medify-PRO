  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold text-foreground">ওষুধের ইতিহাস</h2>
          <p className="text-muted-foreground mt-1">কোন দিন কোন ওষুধ কখন খেয়েছেন তার পূর্ণ ইতিহাস</p>
        </div>
        <div className="flex gap-2">
          {[7, 14, 30].map(d => (
            <Button key={d} variant={days === d ? "default" : "outline"} size="sm" onClick={() => setDays(d)}
              className={days === d ? "gradient-primary text-[#160500]" : "border-white/[.08] text-muted-foreground"}>
              শেষ {d} দিন
            </Button>
          ))}
        </div>
      ...
      {loading ? (
        <div className="space-y-4">{[1,2,3].map(i => <Skeleton key={i} className="h-24 w-full rounded-2xl glass" />)}</div>
      ) : Object.keys(groupedByDate).length === 0 ? (
        <Card className="glass-card p-12 text-center">
          <CalendarDays className="w-12 h-12 text-muted-foreground mx-auto mb-3 opacity-50" />
          <p className="text-muted-foreground text-sm">কোনো লগ পাওয়া যায়নি। ওষুধ খাওয়ার পর আবার দেখুন।</p>
        </Card>
      ) : (
                ...
                <CardContent className="p-4">
                  ...
                  <div className="space-y-2">
                    {dayLogs.map((log: any) => {
                      ...
                      return (
                        <div key={log.id} className="flex items-center gap-3 p-2 rounded-xl glass border border-white/[.08]">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-foreground">{log.medicine?.name || "Unknown"}</p>
                            <p className="text-xs text-muted-foreground">{log.medicine?.dosage || ""}</p>
                          </div>
