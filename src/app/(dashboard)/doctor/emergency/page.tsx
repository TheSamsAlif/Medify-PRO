  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold text-foreground">জরুরি বিভাগ</h2>
          <p className="text-muted-foreground mt-1">ক্রিটিকাল পেশেন্ট ও জরুরি রিকোয়েস্ট</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={fetchEmergencies} className="border-white/[.08] text-muted-foreground rounded-xl text-xs"><RefreshCw className="w-4 h-4 mr-1" /> রিফ্রেশ</Button>
      ...
      {loading ? (
        <div className="space-y-3">{[1,2,3].map(i => <Skeleton key={i} className="h-28 w-full rounded-xl glass" />)}</div>
        ...
        <Card className="border border-white/[.08] glass-card p-12 text-center">
          ...
          <p className="text-muted-foreground text-lg mb-1">কোনো জরুরি কেস নেই</p>
          <p className="text-xs text-muted-foreground">সব রোগী নিরাপদ আছেন</p>
        </Card>
      ) : (
        <div className="space-y-3">
          {emergencies.map((e, i) => (
            <Card key={i} className={`border ${e.priority === "CRITICAL" ? "border-red-500/30" : "border-amber-500/30"} glass-card`}>
              <CardContent className="p-4">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <h4 className="font-semibold text-lg text-foreground">{e.patientName}</h4>
                    ...
                    {e.message && <p className="text-sm text-foreground mt-2 bg-red-500/10 p-2 rounded-lg">{e.message}</p>}
                  </div>
                  <div className="flex flex-col gap-2">
                    ...
                    {e.latitude && e.longitude && (
                      <a href={`https://www.google.com/maps?q=${e.latitude},${e.longitude}`} target="_blank">
                        <Button variant="outline" className="border-white/[.08] text-muted-foreground rounded-xl text-xs"><MapPin className="w-4 h-4 mr-1" /> লোকেশন</Button>
                      </a>
                    )}
                    {e.emergencyPhone && (
